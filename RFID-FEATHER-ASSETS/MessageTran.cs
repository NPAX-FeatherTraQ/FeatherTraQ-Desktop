...
namespace Reader
{
    ...
    {
        private byte btPacketType;     //Packet header , the default is 0xA0
        private byte btDataLen;        //Packet length , the number of bytes of data packets from behind ' length ' bytes starting , does not contain ' length ' bytes themselves
        private byte btReadId;         //Reader address
        private byte btCmd;            //Packet command code
        private byte[] btAryData;      //Packet command parameters of the command with no arguments
        private byte btCheck;          //Check and , in addition to the checksum byte checksum itself all
