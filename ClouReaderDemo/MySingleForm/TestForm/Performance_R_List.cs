...
{
    public partial class Performance_R_List : BaseOption, IAsynchronousMessage
    {
        ...
        public String _Reader_ConnectID_1 = "";           // 
        public String _Reader_ConnectID_2 = "";           // 
        ...
        private void btn_StartReadTest_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please add the equipment, and then start the test！");
            }
        }
        // 停止测试
        ...
        private void btn_AddDevice_Click(object sender, EventArgs e)
        {
            ...
            if (ari.ShowDialog() == DialogResult.OK) 
            {
                if (lr.AddSingleReader(_AddConnectType, _AddConnectID, this))
                {
                    ...
                    if (lr.readerIDList.Count == 1) { _Reader_ConnectID_1 = lr.readerIDList[0]; }
                    else if (lr.readerIDList.Count == 2) { _Reader_ConnectID_2 = lr.readerIDList[1]; }
                    MessageBox.Show("Add device success！");
                }
                else 
                {
                    MessageBox.Show("Add device Error：Please check connection parameters or whether they have been re added！");
                }
            }
        }
        // 移除设备
        private void btn_RemoveDevice_Click(object sender, EventArgs e)
        {
            if (lb_ReaderList.SelectedItems.Count > 0)
            {
                ...
                if (!String.IsNullOrEmpty(removeReaderID)) 
                {
                    ...
                    {
                        MessageBox.Show("Remove Error，Please re select！");
                    }
                }
            }
            else 
